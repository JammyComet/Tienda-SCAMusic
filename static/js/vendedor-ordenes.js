function crearFilaOrden(orden) {
    ...
    fila.innerHTML = `
        ...
        <td>
            <a
                href="/vendedor/orden-detalle?codigo=${encodeURIComponent(orden.codigo)}"
...
function mostrarOrdenesVendedor() {
    const tabla = document.getElementById("tablaOrdenesVendedor");
    if (!tabla) return;
    ...
    ordenes.forEach(orden => tabla.appendChild(crearFilaOrden(orden)));
}

document.addEventListener("DOMContentLoaded", mostrarOrdenesVendedor);
